...
public class NumberSymbolInputProcessorImpl extends AbstractInputProcessor implements INumberSymbolInputProcessor {
    ...
    protected final INumberSymbol produceBaseNumberSymbolForValue(int value) {
        return BaseNumbers.getBaseNumberSymbolIDForValue(value).getBaseNumberSymbol();
    }

    protected final INumberSymbol produceDerivedNumberSymbolForValue(int value) {
        BaseNumbers derivedNumber = BaseNumbers.getBaseNumberSymbolIDForValue(value);
        BaseNumbers root1 = BaseNumbers.getBaseNumberSymbolIDForValue(derivedFromValues[0]);
        BaseNumbers root2 = BaseNumbers.getBaseNumberSymbolIDForValue(derivedFromValues[1]);
        assert(root1 != null);
        assert (root2 != null);
        assert (derivedFromValues[0] != 0);
        return new DerivedNumberSymbol(derivedNumber, root1, root2);
    }

    protected final int singularizeValue(int value) {
        int singularized = value;

        if (!BaseNumbers.isValueBaseNumberSymbol(value)) {
