...
public class RSGFXShaderProgram {
	...
	public RSGFXShaderProgram(String vertexS, String fragmentS, boolean hasTextures, int numTextures) {
		setup(vertexS, fragmentS, hasTextures, numTextures);
	}

	// Takes in ids for files to be read
	public RSGFXShaderProgram(int vID, int fID, Context context, boolean hasTextures, int numTextures) {
	...
	private int loadShader(int shaderType, String source) {
		...
		if (shader != 0) {
			...
			if (compiled[0] == 0) {
				Log.e("Shader", "Could not compile shader " + ((GLES20.GL_FRAGMENT_SHADER  == shaderType) ? "Fragment Shader" : "Vertex Shader") + ":");
