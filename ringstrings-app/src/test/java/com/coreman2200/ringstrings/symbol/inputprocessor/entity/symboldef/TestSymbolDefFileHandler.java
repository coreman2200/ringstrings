...
public class TestSymbolDefFileHandler {
    ...
    public void testMappingsAvailableForFullProfile() {
        LocalProfileSymbolImpl user = produceTestSubject();
        int count = 0;

        Collection<Enum<? extends Enum<?>>> idList = user.symbolIDCollection();
        for (Enum<? extends Enum<?>> id : idList) {
            ISymbolDef sd = mTestFileHandler.produceSymbolDefForSymbol(id);
            if (sd != null) {
                count++;
                System.out.println("Successful mapping  for " + sd.getName());
                System.out.println("Quality: " + sd.getQualities().toString());
            }
        }
