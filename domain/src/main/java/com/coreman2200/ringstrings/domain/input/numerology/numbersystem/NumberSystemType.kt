package com.coreman2200.ringstrings.domain.input.numerology.numbersystem

import com.coreman2200.ringstrings.domain.symbol.symbolinterface.ISymbolID

/**
 * NumberSystemType
 * description
 *
 * Created by Cory Higginbottom on 5/25/15
 * http://github.com/coreman2200
 *
 * Licensed under the GNU General Public License (GPL), Version 2.0.
 * You may not use this file except in compliance with this License.
 *
 * You may obtain a copy of the GPLv2 License at
 * http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
 */
enum class NumberSystemType(system: NumberSystem) : ISymbolID {
    // Supported systems..
    CHALDEAN(ChaldeanNumberSystem()), PYTHAGOREAN(PythagoreanNumberSystem());

    private val numberSystem: NumberSystem = system

    fun numberSystem(): NumberSystem = numberSystem
}
